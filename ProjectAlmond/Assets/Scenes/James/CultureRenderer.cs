...
public class CultureRenderer : MonoBehaviour
{
    ...
    void GenerateCulture()
    {
        if(!culture)
        {
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                cell.transform.position = new Vector3(randPosition.x, petriDishHeight, randPosition.y);
                cell.transform.parent = culture.transform;
                cell.GetComponent<MeshRenderer>().material = cellMaterial;
                cells.Add(cell);
            }
        ...
        List<Vector2> cellPositions = (from cell in cells
                                       select new Vector2(cell.transform.position.x, cell.transform.position.z)).ToList();

        List<float> cellRadii = PerlinNoise(cellPositions, seed, scale, octaves, persistance, lacunarity, offset);

        for (int i = 0; i < cells.Count; i++)
        {
            GameObject cell = cells[i];
            cell.transform.parent = null;
            float cellRadiusModifier = (cellRadii[i] + 0.75f) * 0.5f;
            Vector3 cellScale = new Vector3(cellRadius * cellRadiusModifier, cellRadius * cellRadiusModifier, cellRadius * cellRadiusModifier);
            cell.transform.position += new Vector3(0.0f, cellScale.y / 2.0f, 0.0f);
    ...
    public List<float> PerlinNoise(List<Vector2> positions, int seed, float scale, int octaves, float persistance, float lacunarity, Vector2 offset) {
